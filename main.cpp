...
const void aktualna_informacja(Scena Sce);

const void menu_wyswietl();
...
int main()
{
    ...
    while(flaga==0)
    {
        ...
        case 'd':  ///dodaj robota
        {
                ...
                static int aktualna_ilosc_robotow=2;      //na poczatku mamy 3 roboty ale numerowane od 0
                aktualna_ilosc_robotow++;

                Wektor2D WW;
                cout<<"Podaj polozenie poczatkowe robota: " ;
                cin>>WW;

                Sce->Dodaj_Robota( Lacze , WW);

                Sce->Aktualny_Robot = & Sce->RRR[aktualna_ilosc_robotow];     //ustawia wskaznik na aktualnie dodaawanego robota, dziala tylko dla 3 robotow na starcie

                int kat_pocz;
                cout<<"Podaj poczatkowa orientacje robota: ";
                cin>>kat_pocz;

                Sce->Aktualny_Robot -> Obroc_szybko( kat_pocz, Lacze);
        ...
        case 'g':  ///dodaj przeszkode
        {
            cout<<"Podaj punkt W1 i W3: ";
            cin>>W1>>W3;
            Sce->Dodaj_Przeszkode( Lacze , W1 , W3 );
            Lacze.Rysuj();
